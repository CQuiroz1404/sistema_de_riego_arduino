...
class ArduinoController {
  ...
  static async sendData(req, res) {
    try {
      ...
      await Device.updateLastConnection(device.id, req.ip);
      ...
      if (sensores && Array.isArray(sensores)) {
        for (const sensorData of sensores) {
          const sensor = await Sensor.findById(sensorData.sensor_id);
          
          if (sensor && sensor.dispositivo_id === device.id) {
            await Sensor.addReading(sensorData.sensor_id, sensorData.valor);
      ...
      for (const config of configs) {
        if (config.sensor_id === sensorId) {
          ...
          if (valor < config.umbral_inferior && actuator.estado === 'apagado') {
            // Activar riego
            await Actuator.updateState(config.actuador_id, 'encendido');
          ...
          if (valor > config.umbral_superior && actuator.estado === 'encendido') {
            // Desactivar riego
            await Actuator.updateState(config.actuador_id, 'apagado');
            await Actuator.logEvent({
              dispositivo_id: deviceId,
              actuador_id: config.actuador_id,
              accion: 'fin',
  ...
  static async getCommands(req, res) {
    try {
      ...
      const actuators = await Actuator.findByDeviceId(device.id);
  ...
  static async controlActuator(req, res) {
    try {
      ...
      // Verificar permisos
      const device = await Device.findById(actuator.dispositivo_id);
